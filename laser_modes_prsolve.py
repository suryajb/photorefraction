...
class photorefraction(twm):
    ...
    def func2(self,t,y,w,w_p,k_vary,eps):
        Esp = y[0]
        # b = y[1]
        # self.b = self.thermal_bmode_ss_intracavity(w,w_p,k_vary,eps,b,Esp=Esp)
...
class solve_twm(photorefraction):
    ...
    def intracavity_shg(self,lasers,modes,idx,prev_sol,Esp=None):
        ...
        for k,laser in enumerate(lasers):
            nb = prev_sol[k][1]
#             Xa = -1*K_Ta[k]*((sum(prev_sol[:])[0]+sum(prev_sol[:])[1]*2)-(prev_sol[k][0]+prev_sol[k][1]*2))
#             Xb = -2*K_Tb[k]*((sum(prev_sol[:])[0]+sum(prev_sol[:])[1]*2)-(prev_sol[k][0]+prev_sol[k][1]*2))
            ...
            if ϵ_tot == 0:
                ...
                third = (4*g[k]**4/np.abs(Xb)**2 + np.abs(K_Ta[k])**2 - 
                         4*g[k]**2*np.real(np.conj(K_Ta[k])/Xb))#4*g**4/np.abs(Xb)**2
#                 first = (np.abs(Xa)**2 - 4*nb*np.real(np.conj(Xa)*K_Ta[k]) - 
#                          2*np.real(np.conj(Xa)*g_ea[k]*Esp) + 
#                          4*nb*np.real(g_ea[k]*Esp*np.conj(K_Ta[k])) + 
#                          4*np.abs(K_Ta[k])**2*nb**2 +
#                          np.abs(g_ea[k])**2*np.abs(Esp)**2)
#                 second = (-2*np.real(np.conj(Xa)*K_Ta[k]) + 4*g[k]**2*np.real(np.conj(Xa)/Xb) + 
#                           4*nb*np.abs(K_Ta[k])**2 +
#                           2*np.real(g_ea[k]*Esp*np.conj(K_Ta[k])) - 
#                           8*g[k]**2*nb*np.real(np.conj(K_Ta[k])/Xb) - 
#                           4*g[k]**2*np.real(np.conj(g_ea[k])*np.conj(Esp)/Xb))
#                 third = (-4*g[k]**2*np.real(np.conj(K_Ta[k])/Xb) + np.abs(K_Ta[k])**2 + 
#                          4*g[k]**4/np.abs(Xb)**2)
                try:
                    coeffs = [third[0], second[0], first[0], zeroth[0]] # there are times when initial w_a/b/c are inputted as an array
                except:
...
#             pdb.set_trace()
    ...
    def pr_shg_transmission(self,fn1=None,fn2=None,save=False,single_scan=False,time_scan=False):
...
#     def save_data(self,data,filename1,filename2):
#         pass
